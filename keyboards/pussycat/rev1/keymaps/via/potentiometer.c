...
#define DEFAULT_POT_TOLERANCE 4
...
int PotValue = 0;
int OldPotValue = 0;
uint8_t PotTolerance = DEFAULT_POT_TOLERANCE;

uint32_t PotTimer         = 0;

uint16_t KC_POT_CW = KC_VOLU;
uint16_t KC_POT_CCW = KC_VOLD;
...
void read_pot_keycode(void) {
  uint8_t current_layer = biton32(layer_state);
	KC_POT_CW = keymap_key_to_keycode(current_layer, (keypos_t){.row=3, .col=0});
	KC_POT_CCW = keymap_key_to_keycode(current_layer, (keypos_t){.row=4, .col=0});
  uint16_t KC_POT_TOLERANCE = keymap_key_to_keycode(current_layer, (keypos_t){.row=4, .col=5});
  if (KC_POT_TOLERANCE >= KC_F1 && KC_POT_TOLERANCE <= KC_F10) {
    PotTolerance = KC_POT_TOLERANCE - KC_F1 + 1;
  }
  else {
    PotTolerance = DEFAULT_POT_TOLERANCE;
  }
}
...
void matrix_scan_user(void) {
	read_pot();
  read_pot_keycode();
	if (PotValue - OldPotValue >= PotTolerance) {
    tap_code(KC_POT_CCW);
    OldPotValue = 0;
    if (PotValue >= PotTolerance) { PotValue = PotValue - PotTolerance; }
	}
	else if (PotValue - OldPotValue <= -PotTolerance) {
    tap_code(KC_POT_CW);
    OldPotValue = 0;
    if (PotValue <= -PotTolerance) { PotValue = PotValue + PotTolerance; }
	}
